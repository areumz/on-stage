...
import type { ShaderPattern } from "@/lib/types";
...
void main() {
  ...
  if (uPattern == 0) {
    ...
    // 배율 6.0 — falloff 반경 안에 링을 여러 개(freq=7 기준 약 4~5개) 채워야 "링 여러 개가
    // 반복된다"는 게 눈에 띈다. 배율이 낮으면(예: 2.0) 링이 1~2개뿐이라 그냥 더 밝은 덩어리로
    // 보이고 무늬로 안 읽힌다(1차 시도에서 실측으로 확인된 문제)
    ...
    // wave/ripple과 가장 이질적으로 달라 보인다 — 대비를 셋 중 가장 크게 잡는다
...
type ShaderParams = { pattern: ShaderPattern; freq: number; falloff: number; speed: number };
...
function GlowPlane({ color, shader }: { color: string; shader: ShaderParams }) {
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uPattern: { value: PATTERN_INDEX[shader.pattern] },
    }),
