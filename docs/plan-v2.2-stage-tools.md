...
8. **A탭(아티스트 페이지)에서 스크롤할 때마다 7번 오버레이가 뜨고, 심할 땐 8~9초씩 복구가
   ...
   - **R3F `<Canvas>`는 기본적으로 스크롤할 때마다 컨테이너 크기를 다시 잰다**
     ...
     `resize={{ scroll: false }}`를 추가해 껐다.
     **정정(2026-08-16, 자체 리뷰로 발견)**: 이 항목을 쓴 시점엔 이게 `Scene3D`의 `<Canvas>`
     전체에 걸리는 기본값이었지만, 이후 A탭 격리를 위해 `notifyContextLoss`로 전체 구조를 가르는
     리팩터(위 "진짜 원인을 찾음" 절 참고)를 거치며 이 옵션이 `notifyContextLoss`가 켜진 분기
     **안으로만** 들어갔다. 그 결과 실제로 적용되는 건 B탭(`StageScene`, 스크롤 없는 고정
     패널이라 애초에 이 옵션이 필요 없는 곳)뿐이고, 정작 스크롤이 있는 A탭 4개 씬(히어로·오빗·
     투어·갤러리)엔 적용되지 않는다. 다만 A탭은 그 뒤 찾은 `cachedHasWebGL` 수정만으로 스크롤
     3라운드×3회 재검증에서 경고 0건이 나왔으므로(아래 세 번째 항목), 이 옵션이 A탭에 빠져 있는
     것 자체가 현재 남은 문제는 아니다 — 다만 "기본값으로 추가"라는 이 항목의 원래 서술은 지금
     코드와 안 맞아 정정해둔다.
   - **아티스트 페이지 하나에 캔버스가 처음부터 3개(히어로+투어+갤러리) 동시에 켜져 있었다.**
     ...
     `TourOrbit`/`GalleryHaze`를 마운트하도록 바꿨었다. 한 번 보인 뒤엔 다시 스크롤해서 벗어나도
     언마운트하지 않는 방식이었고, 재검증 당시엔 로드 직후 canvas 2개(히어로 + `rootMargin` 여유로
     미리 뜬 투어) → 갤러리까지 스크롤해야 3개 → 다시 위로 스크롤해도 3개 유지(언마운트 안 됨)를
     확인했었다.
     **정정(2026-08-16, 자체 리뷰로 발견)**: 이 `useInView` 시도는 최종적으로 코드에 남아있지
     않다. 반복되는 스크롤에서 섹션이 뷰포트에 들고 날 때마다 Canvas가 재초기화되어 흰 화면
     깜빡임 회귀를 일으켰고(사용자 리포트: "이런식으로 계속 깜빡거림"), 이후 A탭을 세션 시작
     전 상태로 완전히 되돌리라는 사용자 요청에 따라 `useInView`·`TourSection.tsx`·
     `GallerySection.tsx` 변경 전부 `git checkout --`으로 되돌렸다(`src/lib/hooks.ts`의
     `useInView`도 삭제). 위 재검증 결과는 그 시점 한정 기록이며 현재 코드엔 `useInView`도,
     `TourSection.tsx`/`GallerySection.tsx`의 지연 마운트도 없다 — 아티스트 페이지는 지금도
     로드 즉시 캔버스 3개가 마운트된다. 그런데도 A탭이 정상인 건 순전히 아래 `cachedHasWebGL`
     수정 덕분이다.
...
9. **(2026-08-16) 슬라이더 스로틀이 같은 컴포넌트 안 필드 두 개를 서로 덮어씀.** Task 6 진입 전
   자체 리뷰(subagent)에서 발견: `useThrottledChange`가 값을 통째로 덮어쓰는 방식(`latest.current
   = value`)이었는데, 호출부(`SpotControls`)가 매번 `{ ...spot, [key]: 값 }`으로 전체 객체를
   새로 만들 때 그 `spot`이 100ms 대기 구간 동안엔 아직 커밋 전이라 오래된 값이었다. 그래서 밝기
   슬라이더를 드래그한 직후(100ms 안) 조명각 슬라이더를 드래그하면, 두 번째 호출이 "밝기는 옛날
   값"인 객체로 전체를 다시 만들어 첫 번째 커밋 예정이던 밝기 변경을 조용히 지워버렸다. 스위치
   (on/off)·색상 피커처럼 스로틀 없이 즉시 반영되는 필드와 겹칠 때도 같은 이유로 되돌아가는 경우가
   있었다. `SmokeControls`의 색상 피커는 애초에 스로틀 자체가 안 걸려 있어(드래그 중 계속 `input`
   이벤트를 쏘는 게 range 슬라이더와 같은 부류인데도) 이 세션 내내 쫓던 `webglcontextlost` 유발
   패턴에 노출된 채 한 번도 테스트되지 않은 상태였다.
   **고침**: `useThrottledChange`를 "전체 값 덮어쓰기"에서 "부분 패치(patch) 병합"으로 재설계했다
   (`src/lib/hooks.ts`). 대기 구간 시작 시점의 `current`(props로 받은 최신 확정값)를 커밋 시점까지
   고정해두고, 그 위에 들어오는 패치들을 계속 얕게 병합하다가 커밋 시 한 번에 합쳐서 내보낸다 — 즉
   `{intensity: 900}` 패치와 `{angle: 0.9}` 패치가 같은 100ms 안에 들어오면 최종적으로
   `{...원본spot, intensity: 900, angle: 0.9}`가 나간다. 호출부(`SpotControls`/`SmokeControls`)는
   이제 바뀐 필드만 패치로 넘기면 되고, `StageControls`의 자유 색상 피커(조명 색상)에도 같은 훅을
   새로 적용해 색상 드래그도 스로틀·병합 보호를 받게 했다(스모그 색상 피커는 이미 있던
   `SmokeControls`의 인스턴스를 농도 슬라이더와 공유하도록 바꿈).
   **재검증** (3001 포트 프로덕션 빌드, 로그인 세션): (a) 합성 이벤트로 밝기→조명각을 100ms 안에
   연달아 변경 → 최종 상태에 둘 다 반영(`intensity:900, angle:0.9`) 확인. (b) 실제 마우스 드래그로
   같은 시나리오 재현(밝기 드래그 직후 바로 조명각 드래그) → 역시 둘 다 기본값에서 벗어나 반영,
   `webglcontextlost`/컨텍스트 경고 0건. (c) 스모그 농도→색상을 100ms 안에 연달아 변경 → 둘 다
   반영. (d) 조명 색상 피커를 7단계로 빠르게 연속 변경(실제 hue 드래그 흉내) → 각 패치가 누적
   병합되며 최종적으로 마지막 값 하나로 정확히 수렴, 컨텍스트 경고 0건. `npm test`(23개)·
   `npm run build` 재통과.
...
- [x] **Step 1: §11 5장 체크리스트 7개 항목 전체 재확인** — 새 필드 없는 구버전 저장값 병합 유지 /
...
- [x] **Step 3: `docs/design-v2.md` §11 5장 체크박스를 `[x]`로 갱신**
- [x] **Step 4: `README.md` 갱신** — "2차 로드맵" 항목(197-201행 부근)에 `[x]` +
...
- Step 1의 7개 항목은 이미 각 Task의 검증 노트에서 실제로 확인된 것들을 다시 모은 것 —
  구버전 필드별 폴백·1차 저장값 호환은 `stageState.test.ts`의 단위 테스트(Task 1)로, 슬라이더
  실시간 반영은 Task 3 브라우저 검증으로, 프리셋 재로그인 유지·이름 겹침 confirm→overwrite·
  색상 스와치는 Task 5 Playwright 검증으로, 셰이더 0줄은 `grep`으로 재확인(Task 6 착수 시
  `ShaderMaterial`/`glsl` 검색 — `src/components/three/HeroBackground.tsx`에 1건 걸리지만 이건
  1차부터 있던 히어로 배경 셰이더로 이번 5장 작업 범위 밖, `StageScene.tsx`엔 0건).
- README는 "무대 연출 툴은 조명 프리셋·on/off·카메라 앵글 전환만 지원..." 알려진 제한사항 항목을
  통째로 제거(이제 사실이 아님)하고, 2차 로드맵 체크박스를 `[x]` + `docs/plan-v2.2-stage-tools.md`
  링크로 갱신, 영문 대응 문단(Known Limitations·Phase 2 progress) 둘 다 국문과 어긋나지 않게 같이
  고쳤다. 원래 로드맵 항목 문구에 있던 "파티클"은 이번 5장 작업 범위가 아니었음을 확인하고(스모그는
  three.js fog + drei `Cloud`로만 구현, 별도 파티클 시스템 추가 없음) 문구에서 뺐다.
- `npm test`(23개) · `npm run build` 최종 재통과.
