...
- [ ] 375~428px(폰)·768px(md)·1024px(lg) 각각에서 A탭 홈 궤도 씬이 6명 아티스트를 프레임 안에 담는다
- [ ] 아티스트 페이지에서 어느 폭에서도 페이지 레벨 가로 스크롤이 없다(커버플로우 포함)
- [ ] `md` 미만에서 B탭 사이드바가 햄버거+드로워로 전환되고, 어느 화면에서도 로그아웃 버튼이 스크롤 없이 보인다
- [ ] `md` 미만에서 무대 연출 툴 컨트롤 패널이 하단 시트로 전환되고, 시트를 열고 닫아도 3D 씬이 `webglcontextlost` 없이 유지된다
- [ ] B탭 그리드(갤러리/지표 카드/투어/대시보드 하단)가 `md` 미만에서 1열로 붕괴한다
- [ ] DPR 클램프(`[1,2]`, StageScene `[1,1.5]`) 적용 후 실기기 체감 프레임레이트가 이전보다 나쁘지 않다 — 이 환경은 실제 모바일 GPU가 없어 검증 불가, 배포 후 확인 필요
