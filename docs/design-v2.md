...
브레이크포인트는 Tailwind 기본값(`sm` 640 / `md` 768 / `lg` 1024)을 그대로 쓰고 커스텀 값은 추가하지
않는다. **2026-08-23 별도 브레인스토밍**(4장·7장 구현 완료 후)에서 배포본을 Playwright + 모바일
뷰포트 에뮬레이션(375/390/428/768/1024, 데스크톱 Chrome 채널 — 실제 모바일 GPU가 없어 프레임레이트가
아니라 레이아웃·시각 밀도만 실측)으로 확인한 결과, 실제 문제들이 정확히 이 세 경계선에 걸려서
커스텀 브레이크포인트 없이 그대로 쓸 수 있다고 판단했다.

대응 우선순위(기존 그대로): A탭 홈 → 아티스트 페이지 → B탭 대시보드 → 무대 툴.

**모바일 3D 전략: R3F 유지, 정적 폴백 없음.** 최적화는 5개 R3F 씬이 공유하는 `Scene3D.tsx` 래퍼
한 곳의 DPR 클램프와, 씬별 지오메트리·파티클 조정으로 대응한다(§6.1).

현재 앱 전체에 반응형 클래스가 **0개**였다(`sm:` / `md:` / `lg:` 검색 결과 없음). 1차에서 의도적으로
데스크톱 전용으로 만든 결과이며, 실측으로 확인된 문제와 대응은 §6.1~6.6에 정리한다.

### 6.1 공용 3D 레이어 — Scene3D DPR 클램프

5개 R3F 씬(`OrbitScene`/`HeroBackground`/`TourOrbit`/`GalleryHaze`/`StageScene`)이 전부
`Scene3D.tsx`의 `<Canvas>`를 통해서만 렌더링된다 — DPR 클램프를 여기 한 곳에 넣으면 5개 씬 전부에
자동 적용된다.

`Canvas`에 `dpr={[1, 2]}`를 **`{...canvasProps}` 스프레드 앞에** 추가한다. `Scene3D`는 이미
`camera`/`shadows` 등을 `canvasProps`로 받아 그대로 `Canvas`에 넘기는 구조라, 개별 씬이 `dpr`을
명시적으로 넘기면 자연히 오버라이드되는 기존 패턴을 그대로 재사용한다 — 새 prop이나 API를 추가하지
않는다.

`StageScene`은 예외로 `dpr={[1, 1.5]}`를 직접 넘겨 기본값보다 낮은 상한을 쓴다. 스팟라이트 최대
3개(각 `castShadow` + 볼류메트릭 빔) + `Cloud`/`fogExp2` + 그림자맵이 겹쳐서, 언릿
(`meshBasicMaterial`) 위주인 나머지 4개 씬보다 픽셀당 연산량이 뚜렷이 많다는 코드 구조 근거로
처음 정한 값이다.

**실측 근거(2026-08-31, §6.6 하단 시트 구현 후 재확인)**: 로컬 프로덕션 빌드를 Playwright
`deviceScaleFactor: 3`(실제 고밀도 모바일 화면 근사) 컨텍스트로 375px 뷰포트에서 열어, 스팟라이트
3개 전원 켜기 + 밝기 최대 + 스모그 농도 0.6으로 맞춘 뒤 `dpr={[1, 1.5]}`와 `dpr={[1, 2]}`를 번갈아
스크린샷 비교했다 — 스팟라이트 글로우 형태·Cloud 텍스처·그림자 경계 모두 두 값 사이에 육안으로
구분되는 차이가 없었다. `1.5`를 그대로 유지한다. 다만 이 환경엔 실제 모바일 GPU가 없어 **프레임레이트는
여전히 검증하지 못했다** — 배포 후 실기기 확인이 필요하다(GalleryHaze와 동일한 전제).

**실측 근거(GalleryHaze만 해당)**: 로컬 프로덕션 빌드(`next build` + `next start`)로 클램프
없음(220파티클) / `[1, 1.25]`(60파티클) / `[1, 2]`(150파티클) 세 조합을 실제로 스크린샷 비교했다 —
`[1, 1.25]`는 헤이즈 파티클이 눈에 띄게 휑해졌고, `[1, 2]`는 클램프 없음과 시각적으로 큰 차이 없이
밀도를 유지했다. 다만 이 환경엔 실제 모바일 GPU가 없어 **프레임레이트는 검증하지 못했다** — 배포 후
실기기 확인이 필요하다.

`GalleryHaze`의 `Sparkles count`를 220 → **150**으로 낮춘다(유일한 파티클 사용처).

### 6.2 A탭 홈 — OrbitScene 카메라를 컨테이너 비율에 맞게

궤도 반경(`RING_RADII = [1.4, 2.3, 3.2]`)과 카메라(`position=[0,0,8]`, `fov=50`)가 고정값이라,
컨테이너 가로세로비가 좁아지면(수직 FOV는 고정인데 가로 FOV가 aspect에 비례해 줄어듦) 궤도
바깥쪽 아티스트 노드가 화면 밖으로 잘려나간다. 실측(390px 폭): NOVA·HALO 등 각도가 좌우에 가까운
노드가 가장자리에서 잘림 — 씬이 계속 회전하므로 시간이 지나면 결국 지나가지만, 특정 순간엔 6명 중
일부만 보인다.

브레이크포인트별로 카메라 값을 스텝으로 나누는 대신, `useThree`로 읽은 컨테이너 aspect에서 카메라
z를 동적으로 계산해 궤도 전체가 항상 프레임 안에 들어오게 한다. 특정 픽셀 구간에서만 맞고 그
...
`AlbumSection.tsx`의 앨범 카드가 `translateX(offset*72%) rotateY(...)`로 좌우에 이웃 카드를 살짝
걸치는 커버플로우 연출인데, 감싸는 컨테이너에 `overflow-hidden`이 없어서 **768px(md)에서 페이지
전체가 130px 가로 스크롤**되는 것을 실측 확인했다(`document.documentElement.scrollWidth` 866 vs
`innerWidth` 768). 오프셋 자체는 카드 폭(`h-52 w-52`) 기준 상대값이라 이미 반응형이니 그대로 두고,
컨테이너에 `overflow-hidden`만 추가해 페이지 레벨 가로 스크롤을 막는다.

### 6.4 B탭 사이드바 — `md` 밑에서 드로워로 전환 (+ 로그아웃 sticky 버그 동시 수정)

`Sidebar.tsx`가 `w-56`(224px) 고정이라, 375px 화면에서 폭의 60%를 차지해 남은 콘텐츠 영역이 지표
카드 한글 텍스트가 한 글자씩 세로로 줄바꿈될 만큼 좁아진다(실측: 대시보드 페이지 문서 폭 432px,
뷰포트 375px, 57px 가로 스크롤). `md` 미만에서 사이드바를 숨기고 헤더에 햄버거 버튼 + 오버레이
드로워로 전환한다(대시보드/투어/아티스트/티켓 4화면 공통, `Sidebar.tsx` 한 곳만 수정).

**같은 컴포넌트에서 발견한 별개 버그**: `<aside>`에 `sticky`/`h-screen`이 없어서, 메인 콘텐츠가
뷰포트보다 긴 페이지(투어 일정 등)에서는 flex 컨테이너가 콘텐츠 높이만큼 늘어나고 사이드바도 같이
늘어난다. `mt-auto`로 사이드바 맨 아래 붙은 로그아웃 버튼이 그만큼 화면 아래로 밀려나 안 보인다
(대시보드는 콘텐츠가 짧아 우연히 보였을 뿐). 이번에 사이드바를 다시 손대는 김에 `sticky top-0
h-screen`을 추가해 같이 고친다. 반응형과 무관한 버그지만 같은 파일·같은 Task로 묶는다.

### 6.5 B탭 그리드 — `md` 밑에서 1열

갤러리 그리드·대시보드 지표 카드(`grid-cols-3`), 투어 섹션·대시보드 하단(`grid-cols-2`) 모두 `md`
미만에서 `grid-cols-1`로 붕괴한다. 아티스트 페이지 히어로(`text-[10rem]`)도 `sm` 미만에서
축소한다(예: `text-6xl`) — 정확한 축소값은 구현 중 실측하며 조정한다.

### 6.6 무대 연출 툴 — `md` 밑에서 하단 시트

`StageControls.tsx`가 `w-72`(288px) 고정 사이드 패널이라, 375px 화면에서 3D 씬이 반토막나고 패널
오른쪽이 화면 밖으로 잘린다(실측 스크린샷 확인). `md` 미만에서는 3D 씬을 항상 전체 화면으로 두고,
`StageControls`를 토글 버튼으로 여닫는 하단 시트로 전환한다 — 평소엔 씬 감상에 집중, 조작할 때만
시트를 연다.

시트가 열리고 닫힐 때 `Scene3D`(`StageStudio`)의 컨테이너 크기 자체는 바뀌지 않게 한다(시트는
오버레이) — `Scene3D.tsx`에 이미 나와 있듯, R3F `<Canvas>`가 컨테이너 크기 변화마다 재구성을
시도하며 `webglcontextlost`를 일으킨 전례가 있다. 시트를 레이아웃으로 밀어내는 대신 오버레이로
얹으면 이 문제를 원천적으로 피한다.
...
| 파일 | 작업 |
|---|---|
| `src/components/three/Scene3D.tsx` | 기본 `dpr={[1, 2]}` 추가(spread 앞) |
| `src/components/three/StageScene.tsx` | `Scene3D` 호출부에 `dpr={[1, 1.5]}` 오버라이드 |
| `src/components/three/GalleryHaze.tsx` | `Sparkles count` 220 → 150 |
| `src/components/three/OrbitScene.tsx` | 카메라 z/fov를 컨테이너 aspect 기반 동적 계산으로 변경 |
| `src/components/fans/AlbumSection.tsx` | 커버플로우 컨테이너에 `overflow-hidden` 추가 |
| `src/components/staff/Sidebar.tsx` | `md` 밑 햄버거+드로워 전환, `sticky h-screen` 추가(로그아웃 버그 수정) |
| `src/components/staff/StageStudio.tsx` / `StageControls.tsx` | `md` 밑 하단 시트 전환, 토글 버튼 추가 |
| `src/components/fans/GallerySection.tsx` | 갤러리 `grid-cols-3` → `md` 미만 `grid-cols-1` |
| `src/app/staff/(console)/dashboard/page.tsx` | 지표 카드 `grid-cols-3`, 하단 `grid-cols-2` → `md` 미만 `grid-cols-1` |
| `src/components/fans/TourSection.tsx` | `grid-cols-2` → `md` 미만 `grid-cols-1` |
| `src/app/artists/[slug]/page.tsx` | 히어로 `text-[10rem]` → `sm` 미만 축소(예: `text-6xl`) |
...
- [x] 375~428px(폰)·768px(md)·1024px(lg) 각각에서 A탭 홈 궤도 씬이 6명 아티스트를 프레임 안에 담는다
- [x] 아티스트 페이지에서 어느 폭에서도 페이지 레벨 가로 스크롤이 없다(커버플로우 포함)
- [x] `md` 미만에서 B탭 사이드바가 햄버거+드로워로 전환되고, 어느 화면에서도 로그아웃 버튼이 스크롤 없이 보인다
- [x] `md` 미만에서 무대 연출 툴 컨트롤 패널이 하단 시트로 전환되고, 시트를 열고 닫아도 3D 씬이 `webglcontextlost` 없이 유지된다
- [x] B탭 그리드(갤러리/지표 카드/투어/대시보드 하단)가 `md` 미만에서 1열로 붕괴한다
- [ ] DPR 클램프(`[1,2]`, StageScene `[1,1.5]`) 적용 후 실기기 체감 프레임레이트가 이전보다 나쁘지 않다 — 이 환경은 실제 모바일 GPU가 없어 검증 불가, 배포 후 확인 필요(문서 맨 끝 실기기 체크리스트 참고)
