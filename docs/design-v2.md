...
(`meshBasicMaterial`) 위주인 나머지 4개 씬보다 픽셀당 연산량이 뚜렷이 많다는 **코드 구조 근거로만**
정한 값이다 — GalleryHaze처럼 스크린샷으로 시각 비교하지는 않았다. 브레인스토밍 시점엔 무대
연출 툴 패널이 아직 `w-72` 고정이라(§6.6 이전) 375px 화면에서 3D 씬 자체가 절반쯤 잘려 있어,
DPR 차이를 스크린샷으로 비교해도 판단할 수 있는 화면이 아니었기 때문이다. **따라서 `1.5`라는
구체적 수치는 아직 시각 검증되지 않았다** — §6.6(하단 시트)이 구현돼 3D 씬이 전체 화면으로 보이게
된 뒤에 다시 확인해야 한다(계획 문서 Task 6에서 재확인).
