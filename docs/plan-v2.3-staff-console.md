...
Task 4와 Task 5 사이에 사용자 요청으로 추가. `Sidebar.tsx`에 로그아웃 버튼을 붙였다 —
...
Task 4 완료 후 사용자가 직접 화면을 써보며 나온 후속 수정들.

- "액션" → "수정", "A탭 노출" → "메인 노출" 헤더 문구 조정 (`ToursManager.tsx`)
- 날짜 정렬: 처음엔 표 위 별도 토글 버튼("최신순"/"오래된순")으로 만들었다가, 다가오는 공연 날짜에
  "최신순" 문구가 어색하다는 피드백을 받아 "날짜" 헤더 옆 화살표(▲/▼) 클릭 토글로 교체
- **버그 발견**: 사용자가 테스트 중 aurora 아티스트의 같은 날짜(2026-08-22)에 ICN·LA 두 도시 공연이
  동시에 잡혀 있는 걸 발견 — 시드 스크립트의 해시 기반 날짜 분배 공식이 우연히 같은 날로 반올림한
  기존 데이터(51개 공연 중 1건). 한 아티스트가 같은 날 두 도시에 있는 건 물리적으로 불가능한데
  DB 제약 `unique(artist_id, city_code, show_date)`은 도시가 다르면 막지 못한다.
  - 처음엔 제약을 `unique(artist_id, show_date)`로 좁히는 안(같은 도시 회차도 같이 막힘)과 시드
    스크립트 날짜 공식 수정까지 검토했으나, 사용자가 범위를 좁혀 확정: **DB 제약은 그대로 두고**
