...
- [x] **Step 1: 375/390/428/768/1024px 각각에서 A탭 홈 궤도 씬이 6명 아티스트를 프레임 안에
      담는지 재확인**(Task 2 완료조건의 최종 재검증)
- [x] **Step 2: 아티스트 페이지가 어느 폭에서도 페이지 레벨 가로 스크롤이 없는지 재확인**
- [x] **Step 3: `md` 미만에서 B탭 4화면 전부 드로워로 전환되고, 로그아웃 버튼이 항상 보이는지
      재확인**
- [x] **Step 4: `md` 미만에서 무대 연출 툴이 하단 시트로 전환되고, 개폐 중 `webglcontextlost`가
      없는지 재확인**
- [x] **Step 5: B탭 그리드 4개 화면이 `md` 미만에서 1열로 붕괴하는지 재확인**
- [x] **Step 6: `npm run build`·`npm test` 최종 통과 확인**
- [x] **Step 7: `docs/design-v2.md` §11 6장 체크박스 6개 `[x]`로 갱신** — DPR 항목은 "실기기 확인
      필요"로 남겨둔 전제를 그대로 유지한 채, 이 환경에서 확인 가능한 나머지 5개만 체크하고 DPR
      항목은 배포 후 실기기 확인 전까지 미체크로 남긴다(허위로 체크하지 않는다)
- [x] **Step 8: `README.md` 갱신**(국문·영문 병기, 2차 로드맵 항목 — 6장을 마지막으로 2차 로드맵
      전체 완료)
...
**검증 노트**: `next build && next start -p 3001`(포트 3000 미사용)로 Task 1~6 전체를 한 번에
재확인.

- Step 1: 375/390/428/768/1024px 전부에서 홈 궤도 씬 캔버스가 뷰포트 폭에 정확히 맞춰 렌더되고,
  스크린샷(375px·1024px)으로 6명 아티스트 노드가 전부 원 안에 잘리지 않고 들어오는 것 확인.
- Step 2: `/artists/aurora`를 비-모바일 컨텍스트(뷰포트 폭만 조정, `isMobile` 미사용 — Task 3에서
  확립한 측정 방식)로 5개 폭 전부 `scrollWidth === innerWidth` 확인, 가로 스크롤 없음.
- Step 3: `/staff/dashboard`·`/tours`·`/artists`·`/tickets` 4화면 375px 전부에서 가로 스크롤 없음 +
  햄버거로 드로워 열면 로그아웃 버튼 보임 확인. `/staff/tours`(가장 긴 페이지)를 1280px에서
  1500px 스크롤해도 데스크톱 sticky 사이드바의 로그아웃 버튼이 계속 보임 확인.
- Step 4: 375px에서 씬이 뷰포트 전체 폭으로 렌더, 시트 열기/닫기 3회 반복 후에도
  `webglcontextlost` 콘솔 메시지 0건. 1024px에서 데스크톱 사이드 패널 폭이 여전히 288px(`w-72`)로
  1차 구현과 동일 — 회귀 없음.
- Step 5: `/staff/dashboard`(지표 카드·하단 그리드)·`/artists/aurora`(갤러리·투어) 4개 그리드 모두
  375px에서 1열, 1024px에서 기존 그리드(3/2/3/2열)로 정상 전환 확인.
- Step 6: `npm run build`·`npm test`(32개) 통과.
- Step 7: design-v2.md §11 6장 6개 항목 중 5개 `[x]`, DPR 프레임레이트 1개만 실기기 미검증 사유와
  함께 미체크로 유지.
- Step 8: README.md 국/영문 모두 갱신 — "모바일 반응형 미지원" 알려진 제한사항 항목을 삭제하고
  "모바일 실기기 프레임레이트 미검증" 항목으로 교체(반응형 자체는 구현됐고 실기기 성능만 미확인이라는
  더 정확한 서술), 2차 로드맵 반응형 항목을 `[x]`로 변경하고 구현 내용·실기기 캐비엇을 함께 기술,
  Phase 2 progress 영문 문단에 대응 문장 추가. "갤러리 이미지 정교화"는 design-v2.md §3 시퀀싱(2.1~2.5)
  밖의 별도 검토 항목이라 미체크로 남김(2.1~2.5 정식 로드맵 자체는 이번 6장으로 전체 완료).

`npm run build`·`npm test`(32개) 최종 통과.
...
아니라, Task 7까지 머지·배포된 뒤 실제 모바일 기기로 직접 확인하는 스텝이다.**
