...
**스크롤 스토리 전체 구성** (Scroll based animation 적용)
  ...
  3. 디스코그래피 — 목업 없음, Hero/섹션2와 동일 톤(다크 + 브랜드 컬러 포인트)으로 자유 구성.
     트랙(싱글)별 개별 커버를 3D 커버플로우로 얹고, 트랙 리스트 hover와 연동해 해당 커버가 정면으로 전환된다
...
- 사이드바 잔여 메뉴 실 화면 구현
- 셰이더 심화 (아티스트별 차별화 확대)
- 실제 콘텐츠 이미지 정교화 — 갤러리는 아티스트별로 두 방식이 섞여 있다:
  - AURORA/VELVET/NOVA/LUMEN: Openverse의 CC0/퍼블릭도메인/CC-BY 사진 (출처는
    `public/gallery/CREDITS.json`, CC-BY는 화면 하단에도 촬영자·라이선스 표기)
  - ECHO/HALO: Google Gemini로 생성한 이미지(사용자 직접 생성·제공). 저작권 표기 대상이 아니라
    화면엔 "AI로 생성됨" 한 줄만 표기(`GallerySection`의 `photo.license === "AI"` 분기)
  - `public/gallery/`에 전부 800×1000으로 로컬 저장. 2차에서 나머지 아티스트도 AI 생성 등
    더 정교한 이미지로 교체 검토.
  > 외부 URL 방식(LoremFlickr)은 1차에서 시도했다가 철회했다. 원본이 요청 크기보다 작으면 빨간
  > 레터박스가 섞이고, `lock` 값이 고정 식별자가 아니라 같은 주소가 시간이 지나면 다른 사진을
  > 가리켜 무드가 무너진다. 라이선스도 NC/ND가 섞여 있어 저장소에 담기 부적합하다.
